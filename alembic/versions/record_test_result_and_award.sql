...
BEGIN
    ...
    -- The question_attempts table has unique constraint uq_attempt_idempotency (question_id, user_id, idempotency_key)
    ...
    BEGIN
        INSERT INTO question_attempts(id, question_id, challenge_id, user_id, judge0_token, source_code, stdout, stderr, status_id, status_description, time, memory, is_correct, created_at, idempotency_key, latest)
        VALUES (gen_random_uuid(), p_question_id::uuid, NULL, p_profile_id::integer, NULL, NULL, NULL, NULL, CASE WHEN p_passed THEN 3 ELSE 1 END, CASE WHEN p_passed THEN 'passed_via_rpc' ELSE 'failed_via_rpc' END, NULL, NULL, p_passed, v_now, ('rpc:' || p_test_id || ':' || p_profile_id), true)
        ON CONFLICT ON CONSTRAINT uq_attempt_idempotency
