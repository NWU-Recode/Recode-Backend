...
MAX_TTL = 3600     # 1 hour max for safety
...
class SlideMetadata(BaseModel):
    ...
    has_file: bool = Field(False, description="True if slides_key exists in DB (file available)")
...
async def download_slide(slide_id: int, ttl: int = Query(DEFAULT_TTL, description="TTL for signed download link (seconds)")):
    """
    Generate a fresh signed URL for the slide file referenced by the DB row.
    Frontend should call this when the end user requests a download (so the URL is fresh).
    """
